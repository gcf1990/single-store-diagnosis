...
> **当前增量已发布 q084 测试包，未commit/push（v2.17，2026-08-07）**：Phase 3AV / a310 有效经销商白名单 SQL 字段修复。用户提供线上 `execute-sql-query` 5001 报错，根因为 a310 真实字段是 `父级经销商代码 / 父级经销商简称`，不存在 `一级经销商代码 / 父经销商简称`。本阶段只修 `multi-store-super-app/filter-api.js` 的 `validDealers` SQL 快路径：查询真实字段并别名输出为下游既有 `一级经销商代码 / 父经销商简称`，preview fallback 同时兼容真实字段与别名字段；`index.html` 中 `filter-api.js` cache busting 升级为 `20260807-dealer-parent-field-v217`。不改销售事实、目标、过程、打铁、小订、组织权限、发布目标或共享源码测试/生产映射。真实 a310 SQL 返回 5 行；staging Node `256/256`、lint `62 files`、build PASS、PC `137/137`、audit high `0`、source/dist `35/35`、隐私扫描 `0`、zip 完整性 PASS。测试 App `q0844640cf6734877a3193d6` 已更新，平台回执 `operation=update`、`version=0.1.0`、`artifact=dist.0.1.0.zip`、`fileKey=288c4c84-5d0c-4e2d-9c1b-2461087061b5`；发布目录 `/tmp/multistore-q084-a310-test.pRJUOG/multi-store-super-app`，最终 zip SHA-256=`73a27fa3fed0a29c890898f1adfe9567390b8e02ca8ca65d5169067f2d53e89e`，`186478` bytes，`38` 个 dist 文件；`dist/settings.json` 为 `environment=test` 并跳测试单店 `r8ce093b6d93143d8aa6852f`，同时保留生产单店映射 `aca59d2e2e60f4be4b8b93ac`；共享源码 `settings.json` 保持生产基线。匿名入口 HTTP `401` 仅证明登录保护，未做登录态线上业务 UI 验收，未 commit/push。

### Phase 3AV：a310 有效经销商白名单 SQL 字段修复 — 已发布 q084 测试包，未commit/push

**目标：** 修复 a310 `validDealers` SQL 快路径中使用不存在字段导致 `UNRESOLVED_COLUMN` 的问题，消除首屏白名单快路径必失败和 fallback 噪音。

**交付内容：**
- `filter-api.js` SQL 字段从 `一级经销商代码 / 父经销商简称` 物理字段改为 `父级经销商代码 / 父级经销商简称`，并分别别名回 `一级经销商代码 / 父经销商简称`。
- `toDealer` 兼容 SQL 别名字段和 preview 真实字段，避免 fallback 结果丢父级经销商信息。
- `filter-api-sql-scope.test.mjs` 改为校验真实字段 SQL，避免 mock schema 继续掩盖字段漂移。
- `index.html` 升级 `filter-api.js` cache busting 为 `20260807-dealer-parent-field-v217`。

**门禁结果：** 真实 a310 SQL 返回 5 行；Node `256/256`、lint `62 files`、build PASS、PC `137/137`、audit high `0`、source/dist `35/35`、隐私扫描 `0`、zip 完整性 PASS。已发布测试 App `q0844640cf6734877a3193d6`，`fileKey=288c4c84-5d0c-4e2d-9c1b-2461087061b5`，最终 zip SHA-256=`73a27fa3fed0a29c890898f1adfe9567390b8e02ca8ca65d5169067f2d53e89e`，包内 `environment=test`。

> **当前增量已发布 q084 测试包，未commit/push（v2.16，2026-08-06）**：Phase 3AU / `SCOPE-033 / AC-437～AC-444` 只放开 PC 顶部“销售总览”目标摘要、`销售指标` 三卡和 `过程指标` 四卡的 `validDealers` 二次过滤。顶部 raw summary 必须在品牌、日期、上游大区/小区/经销商、车系和罗盘 RLS 条件下直接汇总 raw 销售事实、raw 目标源和 `monthlyTarget.targetActuals`；订单/零售达成分子不要求 actual 自然键先命中有效门店或已配置目标行。非顶部链路保持 v2.15 既有合同不变，不因顶部 raw 改动扩权或改口径；订单 target-only 明细仍保留既有不与 `validDealerMap` 求交的行为，零售门店目标和销售事实行仍按既有 scoped 规则。实现范围允许 `multi-store-super-app/metrics.js`、`multi-store-super-app/app.js` 最小空态/占位分流和 `metrics-scope`、`monthly-target`、`funnel` 定向测试；不新增查询、不改 `data-api.js` 数据加载、不改目标源、不向其他 app 逻辑扩散。最终证据：Node `240/240`、lint `62 files`、build PASS、PC `137/137`、audit high `0`、`git diff --check` PASS、source/dist `36/36`；Code Review Stage1/2 PASS P0/P1/P2=`0/0/0`；QA PASS P0/P1/P2=`0/0/0`。发布证据：多店 App `q0844640cf6734877a3193d6` 已更新，URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`，平台回执 `operation=update`、`version=0.1.0`、`artifact=dist.0.1.0.zip`、`fileKey=5c91228a-6b73-4071-bd52-6af356e0f8fd`；发布目录 `/tmp/multistore-q084-v216-test.c1uWVY/multi-store-super-app`，zip SHA-256=`02275da1d42035da01a66cb0284eca3a33aac36d56fe5fc443502f9111ef1d0e`，`183353` bytes，`38` 个 dist 文件，`unzip -t` PASS，隐私扫描 `0`；`dist/settings.json` 为 `environment=test` 并跳测试单店 `r8ce093b6d93143d8aa6852f`，同时保留生产单店映射 `aca59d2e2e60f4be4b8b93ac`；共享源码 `settings.json` 保持生产基线。非业务测试稳定性修复边界：fixture-only iron skeleton guard；外链 Playwright route 仅测试隔离，生产链接不变。匿名入口 HTTP `401` 仅证明登录保护，未做登录态线上业务 UI 验收，未 commit/push。

### Phase 3AU：PC 顶部销售总览 raw 汇总范围放开 — 已发布 q084 测试包，未commit/push

**目标：** 落实 `Product-Spec.md` v2.16 / `SCOPE-033 / AC-437～AC-444`。把 `metrics.js` 中 `buildWorkbench` 的计算分成 `raw summary` 与 `scoped details`：顶部目标摘要、销售三卡和过程四卡使用 raw current/month/week 与 raw 月目标汇总；非顶部链路保持 v2.15 既有合同不变，不因顶部 raw 改动扩权或改口径。

**交付内容：**
- 在 `multi-store-super-app/metrics.js` 内保留现有 `scopedRaw(raw, validDealerMap)` 作为明细链路输入，同时新增或重命名清晰的 raw summary 输入，避免顶部和明细共用同一个 scoped 对象。
- 顶部 `salesCurrent / salesPrevious / salesWeek` 改由 raw sales current/month/week 生成；`销售指标` 三卡和 `过程指标` 四卡全部从 raw 分子分母计算，月/周环比同构。
- 顶部订单/零售目标摘要使用 raw 月目标总体汇总；订单/零售达成分子使用 `monthlyTarget.targetActuals` 在上游筛选/RLS 后直接加总，不要求 actual 自然键先命中有效门店或已配置目标行。
- 非顶部链路保持 v2.15 既有合同不变：订单 target-only 明细仍保留既有不与 `validDealerMap` 求交的行为；零售门店目标和销售事实行仍按既有 scoped 规则；门店销售表现、组织下钻、查看全部经销商、门店行、排名/占比、导出、动态诊断、过程分析、打铁、小订和移动端不得跟随顶部 raw 改动扩权或改口径。
- 保持“不限制门店范围”的精确定义：仅不与 `validDealers` 白名单求交；品牌、日期、上游大区/小区/经销商、车系筛选和罗盘 RLS 仍由既有查询返回结果控制。
- `app.js` 只允许做最小空态/占位分流：`hasRawSalesData` 与 `hasTargetDisplayState` 独立判断；无 current raw sales 但目标 loading、单侧 unavailable 或有目标时，标题目标态仍展示且七卡为 `--`；完全无 current raw sales、无目标展示态且无 stores 才整体空态；raw current 有行即使值全 0 也正常显示 `0`。

**关键文件：** `multi-store-super-app/metrics.js`、`multi-store-super-app/app.js`；定向测试为 `multi-store-super-app/validation/metrics-scope.test.mjs`、`multi-store-super-app/validation/monthly-target.test.mjs`、`multi-store-super-app/validation/funnel-metrics.test.mjs`（或既有等价测试文件中的定向用例）。不得修改 `data-api.js`、目标源配置、UI 样式、导出协议、过程/打铁/小订模块、移动端、发布配置或依赖。

**验收门禁：**
1. [x] **Code Review：** 独立核查 raw summary 与 scoped details 已清晰分离；顶部不再读取 `scoped.sales/salesPrev/salesWeek` 或 scoped 月目标汇总；`app.js` 只做 AC-444 所需最小空态/占位分流；没有新增查询、没有 `data-api.js`、数据源和其他 app 逻辑扩散。结果：Stage1/2 PASS，P0/P1/P2=`0/0/0`。
2. [x] **测试完整性：** `metrics-scope` 定向测试必须构造“raw 含白名单外行、scoped 不含该行”的 fixture，覆盖 current/month/week 顶部值与七卡环比使用 raw；`monthly-target` 定向测试必须覆盖 `monthlyTarget.targetActuals` 中未命中有效门店或未配置目标行的 actual 仍计入顶部达成分子，同时覆盖订单 target-only 明细既有例外、零售门店目标和销售事实行既有 scoped 规则不变；`funnel` 定向测试必须覆盖过程四率 current/month/week 均由 raw 分子分母派生。结果：Node `240/240`。
3. [x] **状态/空态回归：** 定向测试必须覆盖 AC-444：无 current raw sales 但目标 loading、单侧 unavailable 或有目标时标题目标态仍展示、七卡为 `--`；完全无 current raw sales、无目标展示态且无 stores 才整体空态；raw current 有行且值全 0 时正常显示 `0`。结果：QA PASS，P0/P1/P2=`0/0/0`。
4. [x] **PC 行为回归：** PC 定向测试必须覆盖上游大区/小区/经销商筛选、车系筛选和 RLS fixture 仍限制 raw 输入，顶部 raw 变化不改变下方组织层级、查看全部经销商、导出、动态诊断、过程分析、打铁、小订或移动端入口。结果：PC `137/137`。
5. [x] **编译与安全验证：** 执行 `(cd multi-store-super-app && npm test && npm run lint && npm run build && npm run test:pc)`；必须额外用 diff 审查证明没有 `data-api.js`、发布配置、依赖或其他 app 逻辑扩散。结果：lint `62 files`、build PASS、audit high `0`、`git diff --check` PASS、source/dist `36/36`。

**门禁结果：** Node `240/240`、lint `62 files`、build PASS、PC `137/137`、audit high `0`、`git diff --check` PASS、source/dist `36/36`；Code Review Stage1/2 PASS，P0/P1/P2=`0/0/0`；QA PASS，P0/P1/P2=`0/0/0`。发布前隔离 staging 复验为 build PASS、Node `240/240`、lint `62 files`、PC `137/137`、audit high `0`、source/dist `36/36`、隐私扫描 `0`、zip 完整性 PASS。多店 App `q0844640cf6734877a3193d6` 已更新，平台回执 `operation=update`、`version=0.1.0`、`artifact=dist.0.1.0.zip`、`fileKey=5c91228a-6b73-4071-bd52-6af356e0f8fd`，URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`；发布目录 `/tmp/multistore-q084-v216-test.c1uWVY/multi-store-super-app`，zip SHA-256=`02275da1d42035da01a66cb0284eca3a33aac36d56fe5fc443502f9111ef1d0e`、`183353` bytes、`38` 个 dist 文件，`dist/settings.json` 为 `environment=test` 并跳测试单店 `r8ce093b6d93143d8aa6852f`，同时保留生产单店映射 `aca59d2e2e60f4be4b8b93ac`；共享源码 `settings.json` 保持生产基线。匿名入口 HTTP `401` 仅证明登录保护；未做登录态线上业务 UI 验收，未 commit/push。非业务测试稳定性修复边界：fixture-only iron skeleton guard；外链 Playwright route 仅测试隔离，生产链接不变。

**停止条件：** 如果需要新增查询、修改 `data-api.js`、改变目标源、改变 v2.15 非顶部既有合同，或需要绕过罗盘 RLS 才能对齐指标表直接加总，立即停止并回到需求确认。

> **当前增量本地修复、Code Review/QA 完成，未发布/未commit/push（v2.15，2026-08-01）**：Phase 3AT / `REQ-014 / AC-428～AC-436` 只修 MG 07 小订在投资人入口、空 `dealerCode` / 空组织筛选下实际留存被汇总为 `0` 的问题。投资人范围允许目标 DS RLS 返回 5 店目标合计 `507`；实际源在同一投资人可见范围内，以固定 `品牌名称=MG`、`汇报车系名称='MG 07'`、固定小订期 `2026-07-29`～`2026-08-22` 汇总，一级经销商代码与叶级门店/投资人门店范围完成可证明映射后，本地 5 店案例为目标 `507`、累计小订 `110`、留存小订 `108`、退订小订 `2`。本阶段未改销售模块、三项小订独立指标、留存作为达成分子、今日新增、目标 source-row summary、无目标门店实际计入父级、顶部日期/车系独立、订单/零售目标、过程、打铁、导出、移动端、发布配置或依赖；未新增小订专用 RLS、授权缓存或角色推断；目标分母未被 `organizationRows`、valid primary、全量经销商维表或 canonical 可归因集合扣减；目标/实际/RLS/字段/映射不可证明时不得静默伪 `0`，必须进入既有数据不完整/降级状态。门禁：Node `235/235`、small-order Playwright `21/21`、lint PASS、build PASS、PC 二次 `137/137`，Code Review/QA 完成；缓存版本串 P1 已修复并验证，`index.html` 与 `dist/index.html` 三脚本均为 `20260801-mg07-investor-scope-v215`，index-version `3/3`、build PASS。当前未发布、未 commit/push，线上投资人登录态未验收。

### Phase 3AT：MG 07 小订投资人空组织筛选实际汇总修复 — 本地完成，未发布

**目标：** 修复投资人进入 `MG 07小订战报` 时，URL 组织筛选为空但目标表 RLS 已限定其可见门店范围的场景。目标摘要继续显示目标 DS RLS 可见且匹配顶部范围的 source-row summary；实际留存按同一投资人可见范围汇总，避免因 `a310` 经销商维表 `经销商代码` 与销售事实 `一级经销商代码` 不一致而错误显示 `0`。

**已通过 AC：**
- [x] AC-428：投资人 URL `dealerCode` 为空、组织筛选为空且目标 DS RLS 返回 5 店目标合计 `507` 时，小订摘要 `小订目标` 显示 `507`，不得显示 `0`。
- [x] AC-429：同一投资人可见范围内，实际源固定 `品牌名称=MG`、`汇报车系名称='MG 07'`、固定小订期汇总；一级经销商代码与叶级门店/投资人门店范围完成可证明映射后，累计/留存/退订显示 `110/108/2`。
- [x] AC-430：`a310.经销商代码` 与小订事实 `一级经销商代码` 不一致时，不得直接以字段相等失败作为实际为 `0` 的理由；必须走既定一级/叶级映射，或在证据不足时 fail-closed 为数据不完整。
- [x] AC-431：目标分母继续按目标 DS RLS 可见且匹配顶部范围的目标源行汇总；`organizationRows` 只做下钻归因、对象名称补全和审计，不得扣减 5 店目标 `507`。
- [x] AC-432：小订实际车系固定 `汇报车系名称='MG 07'`，不得被顶部车系多选或 `MG07 EV/MG07 DMH/MG7` 改写。
- [x] AC-433：小订实际日期固定 `2026-07-29`～`2026-08-22`，不得跟随顶部销售日期。
- [x] AC-434：未配目标但有实际的门店实际继续计入父级累计和 summary 达成分子；门店层“未设目标/--”、不判落后、不显示风险的语义不变。
- [x] AC-435：目标、实际、RLS、字段或映射不可证明时不得静默伪造 `0`；必须进入小订既有数据不完整/降级状态，且业务页面不展示技术 key、RLS 细节或底层异常堆栈。
- [x] AC-436：回归证明销售模块、三项小订独立指标、留存分子、今日新增、目标 source-row summary、顶部日期/车系独立、无新增 RLS/授权缓存/角色推断均未变化。

**实施边界：** 预计只允许触达 `multi-store-super-app/small-order-api.js`、`small-order-model.js`、`small-order-contract.js`、`small-order-view.js`、`app.js`、`organization-view.js`、必要静态版本串和 `validation/small-order-*.test.mjs` / `validation/small-order-*.spec.js`。若修复需要改销售 SQL、订单/零售目标、过程、打铁、导出、移动端、发布配置或依赖，必须停止并重新回需求确认。

**门禁结果：** 已覆盖投资人空组织筛选 fixture、5 店 `507/110/108/2` 数据案例、一级/叶级映射、字段不一致不伪 0、目标不被 `organizationRows` 扣减、固定 MG 07、固定小订期、未配目标实际、scope 内 0 命中/多映射降级、scope 外异常不污染和非目标回归。通过 Node `235/235`、small-order Playwright `21/21`、lint PASS、build PASS、PC 二次 `137/137`、Code Review 和独立 QA；缓存版本串 P1 已修复并验证，`index.html` 与 `dist/index.html` 三脚本均为 `20260801-mg07-investor-scope-v215`，index-version `3/3`、build PASS。未发布、未 commit/push，线上投资人登录态未验收。

> **当前增量已发布生产 re37，发布包 QA PASS，未commit/push（v2.14，2026-07-31）**：本次仅补充 Phase 3AS / `REQ-012 / AC-423～AC-427` 对应 v2.13 代码的生产发布事实，不改 PC 打铁指标 CSV 宽表导出合同、不改打铁 11 项公式、取数、筛选、二级 Tab、外链、移动端、依赖或业务口径。发布源为 GitHub fresh clone `xiaoxia0026/retail-smart-sales-process-multiStores` 分支 `feature/260731-PC`，source HEAD=`372cb1e9a560985a69520975caaf3762d76e9373`，发布目录 `/private/tmp/multistore-prod-release.2Z9nvC`；root/dist/zip settings 均为 `environment=production`。生产 App `re37c3447cb0443a68a36a40` 已更新，URL=`https://rdata-pv.rauto.com/open-apps/re37c3447cb0443a68a36a40/`；跳转单店为生产 App `aca59d2e2e60f4be4b8b93ac`，生产单店 URL=`https://rdata-pv.rauto.com/open-apps/aca59d2e2e60f4be4b8b93ac/`。cache busting 使用 `iron-metrics-view.js?v=20260731-iron-export-v213` 和 `app.js?v=20260731-iron-export-v213`。发布命令为 `guancli app publish --app-id re37c3447cb0443a68a36a40 --path /private/tmp/multistore-prod-release.2Z9nvC -f json`，平台回执 `operation=update`、`appId=re37c3447cb0443a68a36a40`、`version=0.1.0`、`artifact=dist.0.1.0.zip`、`fileKey=f88698dc-cd1a-4ed9-95f8-0b345f394a65`。发布包 SHA-256=`8127f178b73d8b88bc16ba47847adf13c76fc5a4c932aeec576a99ef63ee8554`，`182174` bytes，`38 files`，`unzip -t` PASS；zip 未包含 `node_modules/test-results/playwright-report/validation`，敏感扫描无命中。门禁：`npm ci` PASS，Node `231/231`，PC `137/137`，lint `62 files`，build PASS，audit critical `0`；独立 QA PASS，P0/P1=`0/0`，P2=`1` 仅登录态线上 UI/HAR 未验收。匿名生产入口 HTTP `401` 仅证明登录保护，不代表登录态业务 UI 验收；未创建新 App，未发布测试 App，未 commit/push 本地文档。

> **当前增量已发布 q084，发布包 QA PASS，未commit/push（v2.13，2026-07-31）**：Phase 3AS / `REQ-012 / AC-423～AC-427` 只同步 PC 打铁指标 CSV 宽表导出发布状态，不改打铁 11 项公式、取数、筛选、二级 Tab、外链、移动端、发布配置或依赖。发布证据：测试 App `q0844640cf6734877a3193d6` 已更新，URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`，平台回执 `operation=update`、`version=0.1.0`、`artifact=dist.0.1.0.zip`、`fileKey=8cbbaca1-270e-4b3b-9a8a-4446c50d2fe0`；发布包 `/private/tmp/multistore-q084-iron-export.uQuVLL/dist.0.1.0.zip`，SHA-256=`83d68974f1bb73203d73ecefbbb33ece4533bfc458f441018b02f09ca0f64215`，`182174` bytes，`38 files`，`unzip -t` PASS，未包含 `node_modules/test-results/playwright-report/validation`。zip 内 `dist/settings.json` 为 `environment=test`，测试单店 URL 为 `r8ce093b6d93143d8aa6852f`；共享源码 `multi-store-super-app/settings.json` 保持 `environment=production`；`iron-metrics-contract/model/api/view.js` 和 `app.js` cache busting 使用 `20260731-iron-export-v213`。本地门禁 Node `231/231`、PC `137/137`、lint `62 files`、build PASS、audit critical `0`；Code Review PASS P0/P1/P2=`0/0/0`，发布 QA PASS P0/P1=`0/0`，P2 仅登录态线上 UI/HAR 未验收与文档记录。匿名 URL HTTP `401` 仅证明登录保护，不代表登录态业务 UI 验收；未发布生产。

> **当前增量本地修复、两阶段 Code Review、独立 QA 完成，未发布/未commit/push（v2.09，2026-07-30）**：Phase 3AR / `REQ-010 / SCOPE-032 / AC-413～AC-416` 只修 MG 07 在邀约和试驾接待过程链路的来源级静态车系映射。销售筛选项 `MG 07` 必须在四个运行时打标来源中同时命中原始值 `MG 07`、`MG07 EV`、`MG07 DMH`，且明确排除 `MG7`。四源字段固定为：邀约历史 `n418e47dacdb94291993d3d9.周期首次意向闭环车系名称`、邀约实时 `ta1978fc86ae745009d0eff4.周期首次意向闭环车系名称`、试驾历史 `g9da02067b8a6432486f58f9.车系名称`、试驾实时 `ie2f283f63154402282c4968.闭环车系`；实时试驾的 `车系名称` 仅作审计对照，不得作为过滤字段。当前/月/周三阶段和多选集合必须同构继承；具体车系查询、字段、映射或完整性失败 fail-closed 为 `数据不完整`，不得 fallback 全部车系。非目标：不改 UI、公式、其他车型、数据源、权限、导出、移动端、发布配置或依赖。完成证据：Object.freeze 三别名；定向 `11/11`、Node `230/230`、lint `62`、build PASS、PC `135/135`、source/dist `36/36`；Code Review Stage1/2 PASS，P0/P1/P2=`0/0/0`；QA PASS，P0/P1/P2=`0/0/0`。未发布、未 commit/push。

> **当前增量已发布 q084，未commit（v2.08，2026-07-29）**：Phase 3AQ / `REQ-013 / AC-406～AC-412` 只做 a310 经销商维表 `validDealers` 首屏取数瘦身。默认成功路径使用 `/api/data-source/execute-sql-query`，服务端下推 `开业`、品牌、权威大区、一网/非二网、官网显示名称和上游大区/小区/门店范围，只返回白名单和组织层级所需字段；`preview-with-filter-async/readPreviewFile` 仅 SQL 失败时显式兜底。本阶段不改变权限、白名单、组织、销售过滤、目标、小订、过程、打铁、导出、移动端或共享源码发布配置。完成证据：真实 a310 SQL 可返回样例；专项 `3/3`、Node `228/228`、lint `62 files`、build PASS、PC `135/135`；Code Review 复审 PASS，QA PASS，P0/P1/P2=`0/0/0`；source/dist `filter-api.js` 一致，SHA-256=`93a097a5da0d5fb86f95160dc067decc226ebae74408cc017bbe145293adec07`。发布证据：测试 App `q0844640cf6734877a3193d6` 已于 `2026-07-29` 更新，`operation=update`、`version=0.1.0`、`fileKey=3ac8e19e-7c44-4fd7-b5d2-ff7d073185a3`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`、zip SHA-256=`2b624ff4e2f635991e745a16a02c1adf96613d44736ded80288ab62753ecd2eb`、`182212` bytes、`38` files；zip 内 settings 为 `test`，未包含 `node_modules/test-results`，隐私扫描 `0`。匿名入口 HTTP `401` 仅证明登录保护；线上登录态 UI/HAR 未验收，未 commit/push。

> **当前增量已发布 q084，未commit（v2.06，2026-07-29）**：Phase 3AP / `REQ-013 / AC-397～AC-405` 只做 PC `销售指标` 与默认 `门店销售表现 / 销售概览` 优先首帧。默认全部车系时，车系枚举 pending 不阻塞 `validDealerScope + sales`；带已选车系时仍校验合法性，非法值不得扩大范围。销售首帧前不启动月目标、小订、过程比较期或打铁；销售首帧后月目标、小订、过程 current 分级后台加载；过程 previous/week 首次进入过程 Tab 后加载或等价不抢首帧；初始销售 Tab 不触发打铁 18 次 SQL，首次点击打铁 Tab 才加载且同一 identity 复用。Review-fix 已闭环：诊断 I/O 移到销售首帧后，过程 current 不刷新动态诊断，生产测试 hooks 补齐门禁。Code Review Stage 1/2 PASS，最终 QA PASS，P0/P1/P2=`0/0/0`；QA 专项首帧 `8/8`、process `8/8`、iron `12/12`、Node `226/226`、lint `61 files`、PC `135/135`、临时 Vite build PASS、audit critical `0`、diff-check PASS、source/dist `app.js` SHA 一致；发布前 staging 复核 Node `225/225`、PC `135/135`、lint `61 files`、build PASS、audit critical `0`、隐私扫描 `0`；1440 浅色销售首屏正常，1280/1440 深色无布局或横向溢出。发布证据：测试 App `q0844640cf6734877a3193d6` 已于 `2026-07-29` 更新，`operation=update`、`version=0.1.0`、`fileKey=6a73b471-908a-4368-b3b8-485ae051a2c5`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`、zip SHA-256=`25afbf62737eb5f54a7715090056c38d502734db6f50db1b4bee226560496c37`、`181153` bytes、`38` files；staging/zip settings 为 `test`，shared source settings 保持 `production`，仅触达 q084。保留 3 个非阻塞后续技术债：`data-api` option 直接单测、`app.js/data-api.js` 拆分、线上真实 HAR/登录态性能验收。匿名入口 HTTP `401` 仅证明登录保护；线上登录态 UI/HAR 未验收，未 commit/push。

> **当前增量本地开发、回归与独立 QA 完成，未发布/未commit（v2.05，2026-07-29）**：PC 双空人员画像按总部入口加载。用户确认账号 `13917267856` 的 `marketing_userType` 与 `marketing_orgType` 均为空时，页面应按总部入口进入；区域角色由外部壳应用识别，并通过上游大区/小区/门店筛选范围控制可见数据。实施仅调整 `organization-view.js` 角色解析和对应 PC 回归：双空（缺字段、`null`、空字符串或纯空白）解析为 `role=headquarters`；数据范围仍必须受上游 params、罗盘行权限和有效经销商白名单 `validDealers` 约束；上游 area 自动进入 district 层，上游 district/dealer 自动进入 store 层。非法类型、缺 userType 但非空未知 orgType、非空未知 userType 继续按既有安全策略处理。MG 07 小订不新增独立角色、HQ 或 `nationalComplete` 推断，不改小订目标、摘要、今日新增、达成、排序或下钻。AC-395～AC-396 已关闭；门禁为 Node `225/225`、PC `127/127`、lint `60 files`、build PASS，独立 QA PASS P0/P1/P2=`0/0/0`。未发布、未 commit/push，线上登录态未验收。

> **当前增量已发布 q084（v2.04，2026-07-29，未commit/push）**：Phase 3AO / MG 07 小订今日新增卡与达成表列。用户确认方案 A：`MG 07小订战报` 摘要在 `累计小订` 后新增独立第 3 张卡 `今日新增`，只显示运行时自然日当天小订数并带正号，例如 `+216`，不得加“当日小订”等辅助文案；原 `目标达成`、`时间进度` 和动态层级位顺延，动态层级位成为第 6 张卡。展开后的 `小订达成表现` 表在 `累计小订` 后新增 `今日新增` 列，按 HQ -> 大区 -> 小区 -> 门店逐级汇总；该列仅作当日实绩展示，不参与小订目标、目标达成、时间进度、应达缺口、状态、排序或下钻规则。统计日为运行时自然日且必须落在固定小订期 `2026-07-29`～`2026-08-22`；当天无数据、尚未开始、实际不可用、早于开始或晚于结束均显示 `--`，不得跨期统计；不受顶部销售日期和车系筛选影响。继承 v2.03 当前范围/RLS、source-row summary 目标、全国全范围 `30001`、未归属目标静默审计和独立下钻边界；今日新增字段与日期过滤以 MG07 实际源日期字段反查实现。AC-390～AC-394 已关闭。最终独立 Code Review Stage 1 PASS P0/P1/P2=`0/0/0`；Stage 2 仅 audit 证据曾初始缺失，后续独立 QA `npm audit --audit-level=high` 为 `0`；最终独立 QA PASS P0/P1/P2=`0/0/0`。门禁为 Node `225/225`、PC `127/127`、lint `60 files`、build PASS、source-dist `36/36`、v204 cache、`postcss` lock=`8.5.24`。v2.04 可用截图仍限 `qa-screenshots/mg07-small-order-demo-repair/*fixture*.png` 和 results；旧 `qa-screenshots/mg07-small-order-final/` 为 v2.03 前历史截图，不作为 v2.04 证据。发布证据：测试 App `q0844640cf6734877a3193d6` 已于 `2026-07-29` 更新，`operation=update`、`version=0.1.0`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`、zip SHA-256=`34c6501b408be9ba67ed0c8a10017d6158e8b6032d78a3662143fe4f90ffd2ca`；线上登录态 UI 待验收。

> **当前增量已发布 q084（v2.03，2026-07-29，未commit/push）**：Phase 3AN / MG 07 小订未映射目标静默审计合同已完成代码实现、Review、QA、本地门禁、测试 App 发布和发布后 QA。总部、大区、小区、门店各层级 `小订目标` 仍按目标数据集 RLS 当前可见且匹配当前顶部范围的目标源行 `MG07小订目标` 原始行加总；无法映射到下级组织的目标行仍计入当前层级摘要目标和达成分母，但业务页面不得展示任何未归属目标提示、警告、横幅、角标或状态说明。内部 audit 保留 `originalCode/canonicalCode/dealerName/target/reason/summaryIncluded/drilldownAttributionStatus/handling` 与 `unassignedTargetTotal/unassignedRows`，用于核对子级行合计与 summary 差额。重复目标代码、非法数字、负数、小数、缺字段或当前范围目标总额不可证明等真实阻断错误策略不变。完成证据：Review Stage1/2 PASS P0/P1/P2=`0/0/0`；QA PASS P0/P1/P2=`0/0/0`；Node `221/221`；PC `125/125`；lint `60 files`；build PASS；source-dist 关键文件一致、构建复制清单 `36/36`；隐私 `0`；`npm audit` `0`。发布证据：`operation=update`、`appId=q0844640cf6734877a3193d6`、`version=0.1.0`、`fileKey=157e9036-ff39-484f-a1b1-e771d9e2adf7`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`；实际平台 dist zip=`/private/tmp/mg07-v203-q084-staging.0bY2HJ/dist.0.1.0.zip`，SHA-256=`0b4041b25118404ed44839cfd29a2a5abaa22204e1df434dbf3e9b6e491591af`，`178731` bytes，`38 files`；staging 为 `test`，shared source 保持 `production`，仅触达 q084。发布后 QA PASS P0/P1/P2=`0/0/1`，P2 仅文档状态滞后且已由本次修复；包内旧提示 `0`、Node `38/38`、PC `3/3`、source-dist/隐私通过。在线登录态浏览器核验未完成：Chrome 页签接管超时、IAB 无罗盘登录态；不得写成线上 UI 已验收。未 commit/push。

> **当前增量待开发/未发布（v2.02，2026-07-28）**：重写 Phase 3AN / MG 07 小订目标源行汇总运行时合同。小订模块直接复用应用既有顶部 `brand / region / district / dealer` 范围控制，和销售总览、过程分析、打铁指标使用同一范围；不新增小订专用 RLS、权限集合、授权缓存、角色/HQ/`nationalComplete` 推断。所有层级摘要 `小订目标` 均按目标数据集 RLS 当前可见、且匹配当前顶部范围的目标源行求和，范围匹配优先用目标表 `区域 / MAC / 一级经销商`；不得用 valid primary、全量经销商维表、`organizationRows` 或 canonical 可归因集合的交集删除分母。总部全国若上游明确为“全部大区 + 全部小区 + 全部门店 / 总部全国范围”，摘要目标必须为 `30001`，`29714` 是 v2.01 线上验证发现的 bug，差额 `287` 不得扣除。`organizationRows` 只做下钻归因、对象名称补全和审计；未归属目标行仍计入当前层级摘要目标和达成分母，但不得伪造成子级下钻行，子级行合计可小于摘要目标且差额可审计。累计小订包含当前范围所有 MG 07 小订实际，包括未设目标门店；summary `目标达成 = 累计小订 / 小订目标`，未设目标实际计入父级达成分子。固定小订期、车系独立、未设目标门店层“未设目标/--”且不判落后、重复/非法目标 fail-closed、q084-only 发布边界不变。本阶段仅同步文档，未改源码、未发布、未 commit/push。

> **v2.01 已发布 q084 但被 v2.02 替代（2026-07-28）**：v2.01 完成本地开发、Review、QA 并发布测试 App `q0844640cf6734877a3193d6`，`operation=update`、`fileKey=6c04817e-a964-4c58-a5a6-6728b3f0da34`、zip SHA-256=`eb77189d15f55900ab490d098ce223e1cbfaa842fe1d07b52f06bbdcb3df7fa6`。线上业务验证发现总部全国摘要目标显示 `29714` 而非 `30001`，因此其摘要分母语义不再作为当前开发标准。

> **v2.00 已被 v2.01/v2.02 替代（2026-07-28）**：原“授权一级经销商集合 `S` / 小订独立 RLS / 授权缓存 / HQ 全国完整性推断”描述不再作为当前开发标准，仅保留为历史纠偏记录。

> **当前增量待开发/待复验（v1.99，2026-07-28）**：新增 q084 发布后 CSS 路径与角色兼容合同。刚发布测试 App `q0844640cf6734877a3193d6` 后，线上截图呈现裸 HTML 且出现角色异常 `marketing_orgType 不在已配置范围`。CSS 根因：发布 staging 漏带 `vite.config.ts`，staging build 生成 `dist/index.html` CSS href `/assets/index-DIoWYvru.css`，zip 内有 CSS 但 open-app 子路径下绝对 `/assets/` 不可用；共享正常 `dist` 为 `./assets/...`。角色根因：v1.98 将 `userType=4` 且非 `HQ/RFS/MAC` 的真实非空 `marketing_orgType` fail-closed，导致全应用不可用。实施合同：RFS/MAC 仍优先识别；`userType=4 + 未识别非空 orgType` 必须进入可审计兼容/总部候选态以维持应用加载，但 MG 07 小订全国合同仍只能 `role=headquarters && nationalComplete===true`；`nationalComplete=false` 必须按 RLS `validDealers` scoped 或 fail-closed；未知/缺双字段仍不扩权。发布包必须带 `vite.config.ts` 或直接使用已验证 `dist`，zip 内 `dist/index.html` CSS href 必须为 `./assets/...` 且禁止 `/assets/...`。QA 必须用 `/open-apps/q0844640cf6734877a3193d6/` 子路径模拟，断言 CSS 资源 200/样式生效，并覆盖未识别 orgType + regional RLS 不锁死且不全国 audit。只允许发布 q084，不得触达 `re37`、`x944` 或新 App；登录态线上必须同时确认样式恢复、角色异常消失、小订全国错误横幅消失。本阶段仅同步文档，未改源码、未发布、未 commit/push。

> **当前增量待开发/未发布（v1.98，2026-07-28）**：新增 Phase 3AM / MG 07 小订全国合同双钥与角色识别收紧。线上区域账号仍报全国合同字段 `configuredRows/canonicalUniqueCodes/targetTotal/areaCount/...`，根因锁定为缺 `marketing_userType` 时角色过早默认 HQ、`nationalComplete` 未从 `app.js` 透传至小订链路、合同只凭 `role=headquarters` 进入 `mode=all`、小订静态资源版本串未更新。实施合同：只有 `role=headquarters && nationalComplete===true` 才允许全国 `auditTargets`；HQ 标签但 `nationalComplete !== true` 必须按 RLS `validDealers` / 明确组织证据走受限 scope，无法证明则 fail-closed；`marketing_userType` 缺失但 `marketing_orgType=RFS/MAC` 仍识别为大区/小区；缺 `userType` 且无 `orgType` 不得默认 HQ 用于全国合同。`nationalComplete` 必须进入 `app.js -> small-order-api/cache/raw -> small-order-contract/model` 和 cache identity。发布包必须更新 `organization-view.js`、`app.js`、`small-order-api.js`、`small-order-contract.js`、`small-order-model.js` 静态资源版本参数，建议 `20260728-mg07-rls-v198`。测试矩阵必须覆盖 `userType=4+RFS`、缺 `userType` + `RFS`、缺 `userType` 且无 `orgType` + 区域 RLS、`MAC`、明确 HQ + 全国完整、HQ 标签 + 区域 RLS、缓存版本断言。发布只允许既有测试 App `q0844640cf6734877a3193d6`，不得触达 `re37`、`x944` 或新 App；登录态线上验收必须确认错误横幅消失、区域账号只见本范围，未验不得写完成。本阶段仅同步文档，未改源码、未发布、未 commit/push。

> **当前增量已发布测试App，发布后条件PASS（v1.97，2026-07-28）**：Phase 3AL / MG 07 小订 RLS 范围感知合同修复已完成本地实现、Code Review、独立 QA 代码门禁和隔离测试 App 发布。合同分层固定为：HQ 继续执行全国固定 QA（目标源 `404` 行、排除总计后 `403` 家、目标 `30001`、零目标 `17`、`7` 大区、`MQ257T -> MQ256T`、全国必需样本与全国守恒均保留）；region/district/store 必须先确定 `permissionScope` / `visibleTargets`，再只处理 scope 内目标和实际。scoped contract 已验证 scope source/mapped/unique/targetTotal 守恒、`unmapped=0`；scope 内缺映射 fail-closed，scope 外目标、异常或全国样本缺失不报错、不展示、不计入当前分母。发布回执：测试 App `q0844640cf6734877a3193d6`，`operation=update`、`version=0.1.0`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`、未返回 `fileKey`；zip=`/private/var/folders/3f/mvqcsp_x4qv1g3f8cghnfszh0000gn/T/multistore-v197-release-qbuTUH/multi-store-super-app/dist.0.1.0.zip`，SHA-256=`4c94cad73df49a084dcde855f95cbf2d879b5d4c78f67c8803be7b6629e3b58e`，`176287` bytes，解包 `38 files / 744334 bytes`，`unzip` PASS。staging/zip settings 为 `test`，shared source settings 仍为 `production`（hash=`f76174fe...`）；未触达 `re37`、`x944` 或新 App，未 commit/push。发布后独立 QA 条件 PASS，P0/P1/P2=`0/0/2`，zip/hash/settings/source-dist/RLS 代码/隐私均通过；未完成登录态业务 UI 和线上资源时间核对，需用户已登录刷新确认无 `小订目标合同不守恒` 横幅且区域范围正确；不代表线上验收通过，不回滚不重发。

> **当前增量发布准备中/门禁修复中（v1.96，2026-07-28）**：新增 Phase 3AK / MG 07 修复包恢复测试发布。用户最新指令为 `发布多店测试APP`，本轮承接 ASM-020 的恢复测试发布新需求，不改写 v1.90 q084 production 发布历史。共享源码 `multi-store-super-app/settings.json` 继续保持 `environment=production`；测试配置只允许存在于隔离 staging，staging 内 `settings.environment=test`，门店详情跳测试单店 `r8ce093b6d93143d8aa6852f`，production 映射 `aca59d2e2e60f4be4b8b93ac` 保留。发布目标唯一为测试 App `q0844640cf6734877a3193d6`，不得触达生产多店 `re37c3447cb0443a68a36a40`、旧 `x944c089c3c4249ea925fde6` 或创建新 App。第一轮 staging Node `190` 中 `188` 通过、`2` 项失败，失败原因是旧 production 测试合同与 staging-only test 期望冲突；尚未调用 `guancli`，不得标记发布成功。后续必须在 staging-only 测试中改为 `test` 期望，并全量通过 Node、lint、build、PC Playwright、critical audit、隐私扫描、source-dist 一致、zip/hash 后才可发布；发布后必须记录 `operation=update`、zip/hash、隐私/source-dist、独立 QA，匿名 `401/302` 仅为认证边界。本阶段当前仅同步文档合同，未改源码、未改 settings 源码、未发布、未 commit/push。

> **当前增量本地开发+Review+独立QA完成，未发布/未commit（v1.95，2026-07-27）**：Phase 3AJ 只做 MG 07 小订战报 Demo 视觉纠偏。旧 v1.94 总体 PASS 因未逐项对照 `.superpowers/brainstorm/38050-1785145297/content/mg07-layout-a-status-v7.html` 和用户确认截图，不能作为视觉完成声明；旧 QA 中数据合同、真实 `dsId`、权限、日期、异常语义、独立状态和非目标边界等子项证据保留。v1.95 已实现 Demo 断言页面顺序 `销售总览` 标题/筛选 → `MG 07小订战报` → `销售指标`/`过程指标` → `门店销售表现`，并补齐默认摘要常驻、达成表现收起、独立 Badge、副标题真实数据截止时间、五卡顺序/颜色/留存退订/总对象数、达成/时间进度比较条、`pp` 文案、蓝实际条、橙时间刻度、带对象数和箭头的展开/收起按钮、1280/1366/1440 浅深主题六张 fixture 实现截图。回修后门禁已通过 Node `190/190`、lint `57 files`、build PASS、PC Playwright `116/116`、critical audit `0`、diff-check PASS、source/dist 一致和隐私扫描；actual-only 实际行 `data_updated_at` 边界已修复，深色主题对比度为 `8.04/7.81`。最终独立 QA 已通过，P0/P1/P2=`0/0/0`；未发布、未 commit，线上登录态与 16 组 READER 未执行；未 commit/push。
...
**交付内容：**
- 统一解析人员画像角色，实现总部/大区/小区/销售总监/投资人入口矩阵；`marketing_userType` 与 `marketing_orgType` 双空（缺字段、`null`、空字符串或纯空白）按总部入口加载；其他关键字段非法、缺 `marketing_userType` 但 `marketing_orgType` 为非空未知枚举、或未知组合显示角色识别异常/既有兼容状态，不得默认扩权。
                                            ...
                                            └─> Phase 3F.3 标题摘要 + 卡内两行环比（已发布测试 App）
                                                  └─> Phase 3G PC 车系筛选多选升级（已完成，Review 与最终 QA PASS；本目标不发布）
                                                        └─> Phase 3H MG 订单/零售目标源切换（已开发，Code Review PASS，最终 QA PASS）
                                                              ...
                                                              ├─> Phase 3AF～3AI MG 07 小订战报数据/状态子项（旧证据保留，视觉总体 PASS 撤回）
                                                              ├─> Phase 3AJ MG 07 小订战报 Demo 视觉纠偏（实现与 Review 回修完成，待重审/QA）
                                                              ├─> Phase 3AK～3AM MG 07 测试发布、RLS 与角色合同后续修复（待开发/待复验）
                                                              ├─> Phase 3AN MG 07 小订目标源行汇总运行时合同（本地完成/待发布）
                                                              ├─> Phase 3AO MG 07 小订今日新增卡与达成表列（已发布 q084，发布后 QA PASS）
                                                              ├─> Phase 3AR MG 07 过程车系别名映射修复（本地修复、Review、QA完成，未发布/未commit）
...
Phase 3AR 作为 v2.09 P0 已完成本地修复、两阶段 Code Review 与独立 QA：只处理 `MG 07` 在邀约/试驾接待四个过程打标来源中的别名映射缺口，修复 `driveHistoryRaw` 等 raw 来源仅映射 `MG 07` 导致同构条件 0 行的问题。它不覆盖、重排或阻塞 Phase 3AE 的整体过程联动合同，不改销售闭集、不改实时试驾过滤字段、不改 UI/公式/其他车型/数据源/权限/发布配置；未发布、未 commit/push。
...
### Phase 3AR：MG 07 过程车系别名映射修复 — 本地修复、Review、QA完成，未发布/未commit/push

**目标：** 落实 `Product-Spec.md` v2.09 / `SCOPE-032 / AC-413～AC-416`。把销售筛选项 `MG 07` 在邀约历史、邀约实时、试驾历史、试驾实时四个过程打标来源中统一映射到 `MG 07`、`MG07 EV`、`MG07 DMH` 三个原始值，同时明确排除 `MG7`，修复 raw 来源仅匹配 `MG 07` 导致无样本的问题。

**交付内容：**
- 更新 `multi-store-super-app/data-api.js` 中过程车系来源级静态 mapper / SQL 条件生成：`ip.history`、`ip.realtime`、`drive.history`、`drive.realtime` 四源均支持 `MG 07/MG07 EV/MG07 DMH -> MG 07`，且 `MG7` 不归入 `MG 07`。
- 四源字段边界必须保持不变：邀约历史/实时使用 `周期首次意向闭环车系名称`，试驾历史使用 `车系名称`，试驾实时使用规范过滤字段 `闭环车系`；试驾实时的 `车系名称` 仅可作为审计对照，不能进入 WHERE 过滤。
- 当前、上月同期、上周同期三阶段复用同一映射和同一 fail-closed 条件；多选集合包含 `MG 07` 时必须与其他车系条件按集合并集正确组合。
- 具体车系查询、字段、映射或完整性不可证时，对应来源和阶段 fail-closed 为 `数据不完整`；不得因 `MG 07` raw 别名缺口回退全部车系或无车系 preview 明细。
- 保持非目标不变：不改 UI、公式、其他车型映射、数据源 ID、权限白名单、导出入口、移动端、发布配置、依赖或小订模块。

**关键文件：** `multi-store-super-app/data-api.js`、必要 `multi-store-super-app/validation/process-tags-*.test.mjs` / `vehicle-series*.test.mjs` / `pc-role-drilldown.spec.js`。如需同步构建产物，仅由开发阶段按项目规则生成 `dist/`；本产品方案阶段不改源码、不构建、不发布。

**完成证据：** Object.freeze 三别名；定向 `11/11`；Node `230/230`；lint `62`；build PASS；PC `135/135`；source/dist `36/36`；Code Review Stage1/2 PASS，P0/P1/P2=`0/0/0`；QA PASS，P0/P1/P2=`0/0/0`；未发布、未 commit/push。

**四步门禁：**
1. [x] **Code Review：** 已完成两阶段 Code Review，Stage1/2 PASS，P0/P1/P2=`0/0/0`；核查四源字段未串用、实时试驾仍以 `闭环车系` 过滤、`MG7` 反向排除、无模糊 like/前缀匹配、无具体车系 fallback 全部、非目标文件未扩散。
2. [x] **测试完整性：** 已完成定向 `11/11` 与 Node `230/230`；覆盖四源 SQL 条件、`MG 07/MG07 EV/MG07 DMH` 正向命中、`MG7` 反向排除、current/month/week 三阶段、多选集合、查询/完整性失败 fail-closed。
3. [x] **编译与安全验证：** lint `62`、build PASS、PC `135/135`、source/dist `36/36`；确认 UI、公式、其他车型、权限、导出、移动端、小订和发布配置无变化。
4. [x] **QA：** 独立 QA PASS，P0/P1/P2=`0/0/0`；已按真实字段样本复核邀约历史 `MG 07`、邀约实时 `MG07 EV/MG07 DMH`、试驾历史 `MG07 EV`、试驾实时 `闭环车系=MG 07 / 车系名称=MG07 EV` 的边界。未发布、未 commit/push。

**停止条件：** 任一来源字段不可证、实时试驾需要改用 `车系名称` 才能出数、`MG7` 无法与 `MG 07` 可靠区分、三阶段条件不一致、多选集合条件不可证、具体车系只能靠全部车系 fallback 出数、或需要修改 UI/公式/数据源/权限/发布配置时，立即停止并输出差异清单，不发布、不 commit/push。
...
### Phase 3AJ：MG 07 小订战报 Demo 视觉纠偏 — 本地开发+Review+独立QA完成，未发布/未commit

**目标：** 落实 v1.95 / `REQ-014 / AC-326、AC-327、AC-341、AC-343、AC-346、AC-351、AC-353～AC-358`。以 `.superpowers/brainstorm/38050-1785145297/content/mg07-layout-a-status-v7.html` 和用户确认浅色截图为 canonical Demo 基线，修正 PC 小订战报与 Demo 不一致的问题；保留 v1.94 已验证的数据合同、权限、日期、异常语义、独立状态和非目标边界。

**交付内容：**
- 断言并实现页面 DOM 顺序：`销售总览` 标题/筛选 -> `MG 07小订战报` -> `销售指标`/`过程指标` -> `门店销售表现`。
- 修正战报标题区：独立状态 Badge；副标题固定小订期、真实 `data_updated_at` 和 `独立于销售日期和车系筛选`。
- 修正五卡摘要：顺序固定，目标蓝，累计包含留存/退订，风险达成和落后对象红色，第五动态位显示当前层级落后对象数和总对象数。
- 修正比较条与按钮：`pp` 领先/落后文案、蓝色实际条、橙色时间刻度、右侧带对象数和箭头的展开/收起按钮、`aria-expanded`。
- 补齐 1280/1366/1440 浅色/深色主题六张 fixture 实现截图、DOM 断言和差异说明；Demo 只有一份已确认浅色参考，不得伪造暗色或多宽度 Demo 基线。fixture 或 mock 截图必须明确标识，不能冒充业务真实数据。

**关键文件：** 当前实现证据涉及 `multi-store-super-app/small-order-view.js`、`multi-store-super-app/small-order.css`、`multi-store-super-app/small-order-responsive.css`、`multi-store-super-app/small-order-model.js`、`multi-store-super-app/small-order-api.js`、`multi-store-super-app/small-order-config.js`、`multi-store-super-app/small-order-contract.js`、`multi-store-super-app/validation/small-order-a11y-visual.spec.js`、`multi-store-super-app/validation/small-order-app-integration.spec.js`、`multi-store-super-app/validation/small-order-model.test.mjs`、`multi-store-super-app/validation/small-order-api.test.mjs`、`multi-store-super-app/validation/small-order-real-replay.test.mjs`、`multi-store-super-app/validation/demo-fixture.test.mjs`、`qa-screenshots/mg07-small-order-demo-repair/` 新证据。不得修改旧未跟踪 `VISUAL-PARITY-MATRIX.md` / `VISUAL-PARITY-VERIFY.md`。

**四步门禁：**
1. [x] **Code Review：** 首轮 Review 缺口已回修，最终重审 PASS；actual-only 实际行 `data_updated_at` 边界已修复，深色主题对比度为 `8.04/7.81`；不得复用旧 v1.94 总体 PASS。
2. [x] **测试完整性：** 主控测试已覆盖小订五卡顺序、Badge、副标题、比较条、展开按钮、DOM 顺序、默认收起、会话内展开保留、独立下钻状态和 fixture 标识；回修后 Node 为 `190/190`。
3. [x] **编译与安全验证：** lint Syntax check `57 files`、build PASS、PC Playwright `116/116`、critical audit `0`、diff-check PASS、source/dist 一致和隐私扫描已完成。
4. [x] **功能/视觉 QA：** 1280/1366/1440 浅深六张 fixture 实现截图已保存于 `qa-screenshots/mg07-small-order-demo-repair/`；最终独立 QA PASS，P0/P1/P2=`0/0/0`。本结论不覆盖发布、线上登录态验收或 16 组 `READER` 权限同步。

**停止条件：** Demo 行 49～89 任一视觉/文案/顺序不一致、无法证明真实 `data_updated_at`、actual-only 实际行更新时间边界回归、小订下钻污染主组织状态、fixture 未标识、六张截图不全、深色主题对比度低于已修复证据 `8.04/7.81`、或非目标主链路回归失败时停止，不发布、不 commit/push。

### Phase 3AM：MG 07 小订全国合同双钥与角色识别收紧 — 待开发/未发布

**目标：** 落实 v1.98。修复区域账号被误判为 HQ 全国合同的问题，确保全国合同必须同时满足 `role=headquarters` 与 `nationalComplete===true`，其余情况按 RLS 受限范围或 fail-closed。

**交付内容：**
- 调整 `organization-view.js` 角色识别顺序：缺 `marketing_userType` 时先识别 `marketing_orgType=RFS/MAC`，分别进入大区/小区；未知身份不得默认 HQ 用于全国合同。
- 将 `nationalComplete` 从 `app.js` 透传到 `loadSmallOrder`、`small-order-api` cache/raw payload、`small-order-contract` 和 `small-order-model`，并纳入 cache identity。
- 调整小订合同判定：只有 `role=headquarters && nationalComplete===true` 才走全国 `auditTargets`；HQ 标签但全国完整性不可证时，按 RLS `validDealers` / 明确组织证据走 scoped contract，无法证明则 fail-closed。
- 更新 `index.html` 静态资源版本：`organization-view.js`、`app.js`、`small-order-api.js`、`small-order-contract.js`、`small-order-model.js` 必须使用新版本串，建议 `20260728-mg07-rls-v198`。

**关键文件：** `multi-store-super-app/organization-view.js`、`multi-store-super-app/app.js`、`multi-store-super-app/small-order-api.js`、`multi-store-super-app/small-order-contract.js`、`multi-store-super-app/small-order-model.js`、`multi-store-super-app/index.html`、`multi-store-super-app/validation/small-order-model.test.mjs`、`multi-store-super-app/validation/small-order-api.test.mjs`、`multi-store-super-app/validation/small-order-app-integration.spec.js`、`multi-store-super-app/validation/pc-role-drilldown.spec.js`。

**四步门禁：**
1. [ ] **Code Review：** 核查 `role=headquarters && nationalComplete===true` 双钥、RFS/MAC 缺 userType 识别、RLS scoped fallback、fail-closed、缓存 identity 和资源版本串。
2. [ ] **测试完整性：** 覆盖 `userType=4+RFS`、缺 `userType` + `RFS`、缺 `userType` 且无 `orgType` + 区域 RLS、`MAC`、明确 HQ + 全国完整、HQ 标签 + 区域 RLS、缓存版本断言。
3. [ ] **编译与安全验证：** 通过 Node、lint、build、PC Playwright、critical audit、隐私扫描、source-dist 一致和 zip/hash。
4. [ ] **发布与线上验收：** 只允许发布测试 App `q0844640cf6734877a3193d6`，不得触达 `re37`、`x944` 或新 App；登录态线上验收必须确认错误横幅消失、区域账号只见本范围，未验不得写完成。

**停止条件：** `nationalComplete` 任一链路未透传、HQ 标签可绕过 RLS、未知身份默认 HQ、RFS/MAC 缺 userType 识别失败、缓存复用旧全国合同、静态资源版本未更新、发布目标不是 `q0844640cf6734877a3193d6`、或登录态区域账号未验收时停止，不得发布完成声明。

### Phase 3AN：MG 07 小订目标源行汇总运行时合同 — 已发布 q084/未线上UI验收/未commit

**目标：** 落实 Product-Spec v2.02/v2.03 / `SCOPE-030 / REQ-014 / AC-372～AC-389`。修复 v2.01 线上全国摘要目标 `29714` 缺陷，把所有层级 `小订目标` 统一为目标数据集 RLS 可见且匹配当前顶部范围的目标源行汇总，替代 valid primary / `organizationRows` 交集扣分母的错误实现；同时确保无法映射到下级组织的目标只做内部审计，业务页面不展示未归属目标提示或警告。状态：本地实现、Review、QA、门禁、测试 App 发布和发布后 QA 已完成；在线登录态浏览器核验未完成，未 commit/push。

**交付内容：**
- 小订目标、实际、摘要、列表和小订下钻直接复用当前顶部 `brand / region / district / dealer` 范围；不得新增小订专用 RLS 请求、授权集合、授权缓存、角色推断、HQ 推断或 `nationalComplete` 推断。
- 小订统计期固定为 `2026-07-29`～`2026-08-22`，不跟随顶部 `startDate/endDate`；车系筛选不影响小订请求、缓存 identity、目标、实际、摘要或列表。
- 所有层级摘要 `小订目标` = 目标数据集 RLS 当前可见、且匹配当前顶部范围的目标源行 `MG07小订目标` 求和；范围匹配优先使用目标表 `区域 / MAC / 一级经销商`。
- 不得用 valid primary、全量经销商维表、`organizationRows` 或 canonical 可归因集合的交集删除摘要分母；总部全国全范围必须显示 `30001`，不得显示 `29714`。
- 累计小订 = 当前顶部范围内所有 MG 07 小订实际之和；目标达成 = 累计小订 / 当前摘要小订目标。
- 未设目标但有实际：父级累计和 summary 达成分子均计入，门店层目标显示“未设目标”、达成率“--”，不显示 `0%`、落后或风险；允许作为未设目标实际行出现在小订达成清单。未设目标且无实际可不出现在清单。
- `organizationRows` 只做 canonical code 和大区/小区/门店下钻归因映射，不作为权限系统、授权集合、可见门店白名单、第二范围来源或摘要分母过滤器。
- 未归属目标行仍计入当前层级摘要目标和达成分母；不得伪造成子级下钻行。下钻列表目标合计可小于摘要目标，差额必须等于未归属/未归因目标并可通过内部审计核对。
- 业务页面不得展示任何未归属目标提示、警告、横幅、角标或状态说明；内部 audit 保留 `unassignedTargetTotal`、`unassignedRows`、摘要计入状态、下钻归因状态和失败原因。
- 顶部范围为空、上游范围不可用或主链路范围不可证时，按现有销售/过程/打铁主链路同构降级；不得回退到全国合同、后台 QA、旧缓存或独立 RLS fallback。
- 重复目标代码或非法数字导致当前范围目标总额不可信时 fail-closed；单行无法唯一归因时保留摘要目标、隔离下钻归因，只写内部审计，不在业务页面展示未归属目标提示。
- 全国 `403/30001/7` 合同只保留为后台管理员数据质量 QA，不阻断业务运行时，不向业务用户展示 `configuredRows/canonicalUniqueCodes/targetTotal/areaCount` 等技术字段。

**关键文件：** `multi-store-super-app/small-order-model.js`、`multi-store-super-app/small-order-contract.js`、`multi-store-super-app/small-order-view.js`、`multi-store-super-app/small-order-api.js`、`multi-store-super-app/organization-view.js`、`multi-store-super-app/app.js`、`multi-store-super-app/index.html`、`multi-store-super-app/validation/small-order-*.test.mjs`、`multi-store-super-app/validation/small-order-*.spec.js`、`multi-store-super-app/validation/pc-role-drilldown.spec.js`。

**四步门禁：**
1. [x] **Code Review：** 核查顶部范围复用、source-row summary、目标表字段范围匹配、固定日期独立、车系独立、无新增授权请求/缓存、`organizationRows` 非分母过滤、无目标语义、错误分级、未归属目标业务页面静默、内部 audit 字段和发布边界。结果：Review Stage1/2 PASS，P0/P1/P2=`0/0/0`。
2. [x] **测试完整性：** 覆盖总部全国 `30001`、大区/小区/门店 source-row summary、`29714` 回归失败用例、日期不联动、车系不联动、未设目标有实际计入 summary 达成分子且门店层“未设目标/--”、无目标无实际、未归属目标摘要计入/下钻差额内部审计、业务页面不展示任何未归属目标提示、范围为空/上游范围不可用降级、重复目标、非法数字、页面不展示技术字段、后台全国 QA 非阻断。结果：Node `221/221`、PC `125/125`，QA PASS P0/P1/P2=`0/0/0`。
3. [x] **编译与安全验证：** 通过 Node、lint、build、PC Playwright、critical audit、隐私扫描、source-dist 一致和 zip/hash；不得篡改 v1.97～v2.01 历史证据。结果：lint `60 files`、build PASS、source-dist 关键文件一致、构建复制清单 `36/36`、隐私 `0`、`npm audit` `0`。
4. [x] **发布与发布后 QA：** 只允许发布测试 App `q0844640cf6734877a3193d6`；shared settings 保持 `production`，staging-only 使用 `test`；不得触达 `re37`、`x944` 或新 App。结果：`operation=update`、`appId=q0844640cf6734877a3193d6`、`version=0.1.0`、`fileKey=157e9036-ff39-484f-a1b1-e771d9e2adf7`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`；zip=`/private/tmp/mg07-v203-q084-staging.0bY2HJ/dist.0.1.0.zip`，SHA-256=`0b4041b25118404ed44839cfd29a2a5abaa22204e1df434dbf3e9b6e491591af`，`178731` bytes，`38 files`；发布后 QA PASS P0/P1/P2=`0/0/1`，P2 仅文档状态滞后且已由本次修复；包内旧提示 `0`、Node `38/38`、PC `3/3`、source-dist/隐私通过。在线登录态浏览器核验未完成，未 commit/push。

**停止条件：** 小订目标或实际不复用顶部范围、总部全国不是 `30001`、仍显示 `29714`、为小订新增独立 RLS/授权缓存/权限集合/角色推断、用 valid primary/全量经销商维表/`organizationRows` 交集删除摘要分母、顶部日期或车系错误影响小订、范围为空时回退全国合同或旧缓存、未设目标门店显示 `0%` 或落后、未归属目标在业务页面出现提示/警告/横幅/角标/状态说明、重复/非法目标被静默吞掉、技术字段暴露给业务用户、或发布目标越界时停止。

### Phase 3AO：MG 07 小订今日新增卡与达成表列 — 已发布 q084/发布后 QA PASS

**目标：** 落实 Product-Spec v2.04 / `REQ-014 / AC-390～AC-394`。在不改变 v2.03 目标、累计、达成、时间进度、状态、排序、下钻和审计口径的前提下，为 `MG 07小订战报` 增加运行时自然日的当日小订展示。

**交付内容：**
- 摘要区从五卡升级为六卡：`小订目标`、`累计小订`、`今日新增`、`目标达成`、`时间进度`、第六动态层级位。动态层级位只顺延，不改 label、总对象数、颜色或层级语义。
- `今日新增` 卡只显示带正号主值，例如 `+216` 或 `+0`；不得展示“当日小订”等辅助小字。无可用当日实绩时显示 `--`。
- 统计日使用运行时自然日，且必须位于固定小订期 `2026-07-29`～`2026-08-22`；早于开始、晚于结束、当天无数据、实际源失败或字段不可证时显示 `--`，不得跨期取数。
- `小订达成表现` 表在 `累计小订` 后新增 `今日新增` 列，按当前列表对象逐级汇总 HQ -> 大区 -> 小区 -> 门店。
- 今日新增只作为展示字段，不参与 `target_total`、`actual_small_order_all`、`achievement_actual`、`achievement_rate`、`period_progress`、`expected_by_time`、`gap_to_expected`、状态判断、默认排序、对象总数或下钻可见性。
- 今日新增继承当前顶部范围/RLS、v2.03 source-row summary、未归属目标静默审计、未设目标实际、独立下钻和错误降级；不受顶部 `startDate/endDate` 与 `vehicleSeries` 影响。
- 不新增数据源，不捏造字段/API；今日新增字段与日期过滤由开发以 MG07 实际源日期字段反查实现。

**关键文件建议：** `multi-store-super-app/small-order-model.js`、`multi-store-super-app/small-order-view.js`、`multi-store-super-app/small-order.css`、`multi-store-super-app/small-order-responsive.css`、`multi-store-super-app/small-order-api.js`、`multi-store-super-app/small-order-contract.js`、必要时 `multi-store-super-app/index.html` 静态资源版本；测试建议覆盖 `validation/small-order-model.test.mjs`、`validation/small-order-api.test.mjs`、`validation/small-order-app-integration.spec.js`、`validation/small-order-a11y-visual.spec.js`、`validation/pc-role-drilldown.spec.js`，以及构建复制清单/source-dist 如发布前需要。

**四步门禁：**
1. [x] **Code Review：** 核查今日新增与累计/达成隔离、运行时自然日边界、固定小订期限制、顶部日期/车系不联动、六卡顺序、表格列顺序、RLS/current scope 继承和无新增数据源。结果：最终独立 Code Review Stage 1 PASS，P0/P1/P2=`0/0/0`；Stage 2 仅 audit 证据曾初始缺失，后续独立 QA `npm audit --audit-level=high` 为 `0`。
2. [x] **测试完整性：** 覆盖期内 `+216` 类正号展示、有效 0 展示 `+0`、期前/期后 `--`、当天无数据 `--`、实际源失败 `--`、顶部日期变化不影响、车系变化不影响、HQ/大区/小区/门店逐级汇总、表格列不参与排序/状态/缺口。结果：最终独立 QA PASS，P0/P1/P2=`0/0/0`。
3. [x] **编译与安全验证：** 通过 Node、lint、build、PC Playwright、audit、source-dist 一致和构建复制清单验证。结果：Node `225/225`、PC `127/127`、lint `60 files`、build PASS、source-dist `36/36`、v204 cache、`postcss` lock=`8.5.24`、`npm audit --audit-level=high` 为 `0`。
4. [ ] **发布与线上验收：** 本阶段尚未发布；仍只允许测试 App `q0844640cf6734877a3193d6`，shared settings 保持 `production`，staging-only 使用 `test`；不得触达 `re37`、`x944` 或新 App。线上登录态未验收。

**停止条件：** 今日新增参与目标达成、时间进度、应达缺口、状态、排序或下钻；统计日取顶部销售日期、跨期补取、期后取 2026-08-22、期前取 0 冒充当日；无数据展示 `+0` 冒充有数；编造源字段/API；或破坏 v2.03 当前范围/RLS、未归属静默审计、全国 `30001`、动态层级位和独立下钻时停止。

### Phase 3AP：PC 销售指标与销售表现优先首帧 — 本地完成，未发布

**目标：** 落实 Product-Spec v2.06 / `REQ-013 / AC-397～AC-405`。只调整 PC 首屏加载编排，让默认打开后的 `销售指标` 与默认 `门店销售表现 / 销售概览` 优先渲染；不改变任何销售、权限、筛选、目标、小订、过程、打铁业务口径。

**交付内容：**
- 重排 PC 初始化关键路径：默认全部车系时，`validDealerScope` 与 sales 核心查询不等待车系枚举完成；车系枚举只负责回填筛选器选项和后续合法性状态。
- 保留具体车系安全门禁：URL 或父应用传入具体 `vehicleSeries` 时，必须先校验已选值合法性，非法值不得扩大为全部车系或绕过销售闭集、组织筛选和 `validDealers`。
- 建立销售首帧事件门禁：销售首帧前不得启动月目标、小订、过程比较期或打铁；销售首帧后再分级启动月目标、小订和过程 current 的后台加载。
- 延后过程比较期：previous/week 比较期首次进入过程 Tab 后启动，或提供等价自动化证据证明其晚于销售首帧且不抢占首帧。
- 打铁懒加载：初始销售 Tab 不启动打铁 18 次 SQL；首次进入 `打铁指标` Tab 才加载，且同一筛选 identity 内复用已有结果或 pending。
- 收紧竞态：销售核心、车系枚举、月目标、小订、过程和打铁均按 `loadToken` / request identity 防写回，筛选变化时必要请求可取消；旧响应不得覆盖新上下文或重复启动 secondary loader。
- 错误态沿用：销售核心失败继续现有销售错误态；不得用目标、小订、过程或打铁状态掩盖销售失败。
- Review-fix 闭环：诊断 I/O 已移到销售首帧后，过程 current 不再刷新动态诊断，生产测试 hooks 已补齐门禁。

**关键文件建议：** `multi-store-super-app/app.js`、`multi-store-super-app/vehicle-series.js`、`multi-store-super-app/data-api.js`、`multi-store-super-app/filter-api.js`、`multi-store-super-app/iron-metrics-api.js`、`multi-store-super-app/iron-metrics-model.js`、`multi-store-super-app/iron-metrics-view.js`、`multi-store-super-app/small-order-api.js`、`multi-store-super-app/small-order-model.js`、`multi-store-super-app/validation/pc-role-drilldown.spec.js`、`multi-store-super-app/validation/vehicle-series.test.mjs`、`multi-store-super-app/validation/vehicle-series-multiselect.test.mjs`、`multi-store-super-app/validation/iron-metrics-app-integration.spec.js`、必要时新增首帧 loader 顺序专项测试文件。

**四步门禁：**
1. [x] **Code Review：** Stage 1 PASS、Stage 2 PASS，P0/P1=`0/0`。审查已核查首帧关键路径、车系枚举默认全部车系解耦、具体车系合法性门禁、secondary loader 启动顺序、打铁懒加载、旧请求防写回，以及销售口径、`validDealers`、筛选、目标、小订、过程、打铁口径、导出、移动端、发布配置和依赖不变。
2. [x] **开发自测完整性：** 专项 `8/8` 已验证 deferred Promise 下车系枚举 pending 时默认全部车系 sales + dealerScope 可完成并渲染、销售首帧早于 secondary loader start、初始销售 Tab 打铁 SQL start 为 0、首次点击打铁 Tab 只启动一次并同 identity 复用、具体车系非法值 fail-closed、过程比较期延后、旧 token 防写回和销售失败错误态。
3. [x] **编译与安全验证：** Node `npm test` `226/226`、lint `61 files`、PC `npm run test:pc` `135/135`、临时 Vite build PASS、audit critical `0`、diff-check PASS、source/dist `app.js` SHA 一致。
4. [x] **最终 QA 与边界确认：** 独立 QA PASS，P0/P1/P2=`0/0/0`；QA 专项首帧 `8/8`、process `8/8`、iron `12/12`；1440 浅色销售首屏正常，1280/1440 深色抽查无布局问题、无横向溢出。线上登录态验收未做，不得写成线上验收通过；本阶段不授权发布、commit 或 push。后续技术债为 `data-api` option 直接单测、`app.js/data-api.js` 拆分、线上真实 HAR/登录态性能验收。

**停止条件：** 默认全部车系仍等待车系枚举；具体非法车系被扩大为全部车系；销售首帧前启动月目标、小订、过程比较期或打铁；初始销售 Tab 触发打铁 18 次 SQL；同一 identity 重复启动同一批打铁 SQL；旧请求覆盖新筛选；销售错误态被 secondary loader 掩盖；或任何销售/权限/筛选/目标/小订/过程/打铁口径变化时停止。

## 5. Product-Spec + Design-Brief P0 覆盖矩阵

| P0 条目 | Phase | 验证证据 |
|---|---:|---|
| SCOPE-033 v2.16 / AC-437～AC-444：PC 顶部“销售总览”目标摘要、销售三卡和过程四卡使用 raw summary，不再与 `validDealers` 求交；订单/零售达成分子为 `monthlyTarget.targetActuals` 在上游筛选/RLS 后直接加总；非顶部链路保持 v2.15 既有合同不变，订单 target-only 明细仍保留既有不与 `validDealerMap` 求交的行为，零售门店目标和销售事实行仍按既有 scoped 规则 | 3AU | 本地完成，未发布/未commit/push；Node `240/240`、lint `62 files`、build PASS、PC `137/137`、audit high `0`、`git diff --check` PASS、source/dist `36/36`；Code Review Stage1/2 PASS P0/P1/P2=`0/0/0`；QA PASS P0/P1/P2=`0/0/0`。非业务测试稳定性修复边界：fixture-only iron skeleton guard；外链 Playwright route 仅测试隔离，生产链接不变；未做登录态线上验证 |
...
| SCOPE-028 / REQ-014 v1.94 子项：PC 独立 `MG 07小订战报` 数据合同、真实 `dsId`、目标 403/30001 守恒、权威维表 canonical 映射、8 家 valid primary 异常状态审计、`MQ257T -> MQ256T`、实际源、异常语义、权限不扩张、固定日期边界和独立状态 | 3AF～3AI | 子项证据保留：Node `187/187`、lint `56 files`、build PASS、PC `111/111`、critical audit `0`、真实回放 `404/1/403`、目标 `30001`、零目标 `17`、`7` 区、`organization_unmapped=0`、`395/8/287`、`MQ257T -> MQ256T`。旧总体 PASS 未对照 Demo，不能作为 AC-346/351 视觉完成声明；尚未发布，未完成登录态生产页面验收，16 组 `READER` 权限同步待用户授权 |
| SCOPE-028 / REQ-014 v1.95 / AC-326、AC-327、AC-341、AC-343、AC-346、AC-351、AC-353～AC-358：MG 07 小订战报 Demo 视觉纠偏，页面顺序为销售总览标题/筛选 -> 战报 -> 销售/过程指标 -> 门店销售表现；默认摘要常驻、达成表现收起、独立 Badge、副标题真实截止时间、五卡顺序/颜色/留存退订/总对象数、比较条、pp 文案、蓝实际条、橙时间刻度、带对象数和箭头的按钮、1280/1366/1440 浅深实现截图和 fixture 标识 | 3AJ | 实现 PASS，Review 回修完成，最终独立 QA PASS；回修后门禁为 Node `190/190`、lint `57 files`、build PASS、PC `116/116`、audit `0`、diff-check PASS、source/dist、隐私扫描。六张实现截图已保存；Demo 只有一份确认浅色基线，不得伪造暗色或多宽度基线。actual-only 更新时间边界已修，深色对比度为 `8.04/7.81` |
| SCOPE-018 / AC-395～AC-396：PC 双空人员画像按总部入口加载，双空含缺字段/null/空字符串/纯空白；上游 area/district/dealer 自动进入 district/store；范围仍由上游 params、罗盘行权限和 `validDealers` 限定；非法类型和非空未知枚举仍安全处理；小订不新增角色/HQ/nationalComplete 推断 | 3A | 本地开发、回归与独立 QA 已完成，AC-395～AC-396 已关闭；实施文件限定 `organization-view.js`、`validation/organization-view.test.mjs`、`validation/pc-role-drilldown.spec.js` 和必要 `index.html` 版本串。门禁 Node `225/225`、PC `127/127`、lint `60 files`、build PASS；独立 QA PASS P0/P1/P2=`0/0/0`。不得修改小订目标、摘要、今日新增、达成、排序或下钻口径；未发布、未 commit/push，线上登录态未验收 |
| SCOPE-029 / REQ-014 v1.98：MG 07 小订全国合同双钥、`nationalComplete` 全链透传、RFS/MAC 缺 userType 角色识别、HQ 标签 + 区域 RLS 不扩权、缓存版本断言、只发 q084 和登录态区域验收 | 3AM | 待开发；必须覆盖 `userType=4+RFS`、缺 `userType` + `RFS`、缺 `userType` 且无 `orgType` + 区域 RLS、`MAC`、明确 HQ + 全国完整、HQ 标签 + 区域 RLS、缓存版本断言。未改源码、未发布、未 commit/push |
| SCOPE-030 / REQ-014 v2.02/v2.03 / AC-372～AC-389：MG 07 小订目标源行汇总运行时合同，顶部范围联动、固定小订日期独立、车系筛选独立、所有层级 source-row summary、summary 达成分子等于累计小订、总部全国 `30001`、未归属目标摘要计入/下钻不归因、业务页面不展示未归属目标提示、内部 audit 保留 `unassignedTargetTotal/unassignedRows`、无目标“未设目标/--”且不判落后、无新增授权请求/缓存、`organizationRows` 不过滤分母、后台全国 QA 非阻断、只发 q084 | 3AN | 已发布 q084，发布后 QA PASS，未 commit/push；v2.02 替代 v2.01 中 valid primary / 组织归因交集错误扣分母的摘要语义，v2.03 补充未归属目标静默审计展示边界，并明确未设目标实际计入父级达成分子。发布证据：`operation=update`、`fileKey=157e9036-ff39-484f-a1b1-e771d9e2adf7`、zip SHA-256=`0b4041b25118404ed44839cfd29a2a5abaa22204e1df434dbf3e9b6e491591af`、`178731` bytes、`38 files`；发布后 QA P0/P1/P2=`0/0/1`，P2 仅文档状态已修。在线登录态浏览器核验未完成，不得写成线上 UI 已验收 |
| REQ-014 v2.04 / AC-390～AC-394：MG 07 小订今日新增卡与达成表列，六卡顺序、今日新增带正号、固定小订期内运行时自然日、期前/期后/无数据/实际不可用 `--`、展开表新增今日新增列、只展示不参与达成/时间进度/缺口/状态/排序/下钻、顶部日期与车系独立 | 3AO | 已发布 q084，发布后 QA PASS，AC-390～AC-394 已关闭；最终独立 Review Stage 1 P0/P1/P2=`0/0/0`，Stage 2 仅 audit 证据曾初始缺失且后续独立 QA `npm audit --audit-level=high` 为 `0`；最终独立 QA P0/P1/P2=`0/0/0`；Node `225/225`、PC `127/127`、lint `60 files`、build PASS、source-dist `36/36`、v204 cache、`postcss` lock=`8.5.24`。发布证据：测试 App `q0844640cf6734877a3193d6` 已于 `2026-07-29` 更新，`operation=update`、`version=0.1.0`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`、zip SHA-256=`34c6501b408be9ba67ed0c8a10017d6158e8b6032d78a3662143fe4f90ffd2ca`；v2.04 截图证据仍限 `qa-screenshots/mg07-small-order-demo-repair/*fixture*.png` 和 results，旧 `mg07-small-order-final/` 不作为 v2.04 证据。线上登录态 UI 待验收，未 commit/push |
| REQ-014 v2.15 / AC-428～AC-436：MG 07 小订投资人空组织筛选实际汇总修复，投资人空 `dealerCode` / 空组织筛选时目标 DS RLS 5 店目标 `507`，实际按同一投资人可见范围汇总，一级经销商代码与叶级门店/投资人门店范围可证明映射，累计/留存/退订 `110/108/2`，字段不一致不伪 `0`，目标不被 `organizationRows` 扣减，固定 MG 07、固定小订期、异常 fail-closed、非目标回归 | 3AT | 本地修复、Code Review 与 QA 完成，AC-428～AC-436 已按本地状态关闭；门禁 Node `235/235`、small-order Playwright `21/21`、lint PASS、build PASS、PC 二次 `137/137`；缓存版本串 P1 已修复并验证，`index.html` 与 `dist/index.html` 三脚本均为 `20260801-mg07-investor-scope-v215`，index-version `3/3`、build PASS。未发布、未 commit/push，线上投资人登录态未验收 |
| REQ-013 v2.06 / AC-397～AC-405：PC 销售指标与销售表现优先首帧，默认全部车系不等车系枚举，销售首帧前不启动月目标/小订/过程比较期/打铁，销售首帧后分级后台加载，初始销售 Tab 不触发打铁 18 SQL，打铁首次点击加载且同 identity 复用，过程比较期延后，旧请求按 token/identity 防写回，销售失败沿用现有错误态 | 3AP | 本地实现、独立 Code Review 与最终 QA 通过，AC-397～AC-405 已关闭；Code Review Stage 1/2 PASS，QA PASS，P0/P1/P2=`0/0/0`；QA 专项首帧 `8/8`、process `8/8`、iron `12/12`、Node `226/226`、lint `61 files`、PC `135/135`、临时 Vite build PASS、audit critical `0`、diff-check PASS、source/dist `app.js` SHA 一致；1440 浅色销售首屏正常，1280/1440 深色无布局或横向溢出。Review-fix 已完成诊断 I/O 移到首帧后、过程 current 不刷新动态诊断、生产测试 hooks 门禁。线上登录态未验收，未发布、未 commit/push；后续技术债为 `data-api` option 直接单测、`app.js/data-api.js` 拆分、线上真实 HAR/登录态性能验收 |
| REQ-013 v2.08 / AC-406～AC-412：a310 经销商维表 SQL 直查首屏瘦身；默认成功路径使用 `/api/data-source/execute-sql-query` 生成 `validDealers`，只查必要字段并下推开业、品牌、权威大区、一网/非二网、官网显示名称、大区/小区/门店范围；preview/readPreviewFile 仅 SQL 失败兜底；白名单、权限、组织、销售过滤和所有业务口径不变 | 3AQ | 本地实现、Code Review 与 QA 通过；实现涉及 `multi-store-super-app/filter-api.js`、`index.html` cache busting、`dist/` 同步和 `validation/filter-api-sql-scope.test.mjs`。已证明 SQL 成功路径不触发 a310 preview/readPreviewFile，SQL 失败才 fallback 且同范围下推，SQL 失败后同 key 下次重试 SQL，0 行与失败状态区分。门禁：真实 a310 SQL 样例返回、专项 `3/3`、Node `228/228`、lint `62 files`、build PASS、PC `135/135`、Code Review PASS、QA PASS、P0/P1/P2=`0/0/0`；未发布、未 commit/push |
...
| `multi-store-super-app/data-api.js` / `validation/process-tags-*.test.mjs` / `validation/vehicle-series*.test.mjs` / `validation/pc-role-drilldown.spec.js` | Phase 3AR MG 07 过程车系别名映射修复 | 已完成本地修复、两阶段 Code Review 与独立 QA；Object.freeze 三别名，定向 `11/11`、Node `230/230`、lint `62`、build PASS、PC `135/135`、source/dist `36/36`，Code Review Stage1/2 PASS、QA PASS，P0/P1/P2=`0/0/0`；未发布、未 commit/push。只改过程标签四源车系 mapper / SQL 条件和对应测试。`MG 07/MG07 EV/MG07 DMH -> MG 07`，`MG7` 反向排除；实时试驾仍以 `闭环车系` 过滤，`车系名称`仅审计。不得改 UI、公式、其他车型、数据源 ID、权限、导出、移动端、小订、发布配置或依赖 |
| `multi-store-super-app/small-order-config.js` / `small-order-api.js` / `small-order-model.js` / `small-order-view.js` / `small-order-contract.js` / `settings.json` / `runtime-config.js` / `app.js` / `index.html` / `visual-sync.css` / `components.css` / `visual-base.css` / `visual-responsive.css` / `validation/small-order-*.test.mjs` / `validation/small-order-*.spec.js` / `validation/pc-role-drilldown.spec.js` | Phase 3AF～3AI MG 07 小订战报 | 旧数据/状态子项证据保留；只证明小订配置、目标/实际 API、canonical 组织映射、异常审计、独立视图状态和部分 UI 门禁。未逐项对照 Demo，不作为 v1.95 视觉完成声明；尚未发布，登录态生产页面验收和 16 组 `READER` 权限同步未完成 |
| `multi-store-super-app/small-order-view.js` / `small-order.css` / `small-order-responsive.css` / `small-order-model.js` / `small-order-api.js` / `small-order-config.js` / `small-order-contract.js` / `validation/small-order-a11y-visual.spec.js` / `validation/small-order-app-integration.spec.js` / `validation/small-order-model.test.mjs` / `validation/small-order-api.test.mjs` / `validation/small-order-real-replay.test.mjs` / `validation/demo-fixture.test.mjs` / `qa-screenshots/mg07-small-order-demo-repair/` | Phase 3AJ MG 07 小订战报 Demo 视觉纠偏 | 本地开发+Review+独立QA完成，未发布/未commit；已覆盖 Demo 视觉、DOM 顺序、卡片/比较条/按钮、actual-only 更新时间边界、深色对比度 `8.04/7.81` 和六张 fixture 截图。不得改数据合同、目标/实际源、权限裁剪、日期边界、异常语义、销售/过程/打铁/导出/单店跳转、依赖、发布配置或旧未跟踪 `VISUAL-PARITY-MATRIX.md` / `VISUAL-PARITY-VERIFY.md` |
| 隔离 staging 副本内的 `multi-store-super-app/settings.json` / 测试期望文件 / 构建产物 `dist/` / `dist.0.1.0.zip` | Phase 3AK MG 07 修复包恢复测试发布 | 发布准备中/门禁修复中；共享源码 settings 继续保持 `environment=production`，仅 staging 内临时改 `environment=test` 并跳转测试单店 `r8ce093b6d93143d8aa6852f`，保留生产映射 `aca59d2e2e60f4be4b8b93ac`。第一轮 staging Node `190` 中 `188` 过、`2` 项失败，原因是旧 production 测试合同冲突；尚未调用 `guancli`。后续必须全量通过 Node/lint/build/PC/audit/隐私/source-dist/zip/hash 后，仅发布测试 App `q0844640cf6734877a3193d6`，不得触达 `re37`、`x944` 或新 App |
| `multi-store-super-app/small-order-model.js` / `small-order-contract.js` / `small-order-view.js` / `small-order-api.js` / `organization-view.js` / `app.js` / `index.html` / `validation/small-order-*.test.mjs` / `validation/small-order-*.spec.js` / `validation/pc-role-drilldown.spec.js` | Phase 3AN MG 07 小订目标源行汇总运行时合同 | 已发布 q084，发布后 QA PASS，未 commit/push；直接复用顶部 `brand / region / district / dealer` 范围输出，所有层级摘要目标按目标数据集 RLS 可见且匹配顶部范围的目标源行求和；summary 达成分子等于累计小订并包含未设目标实际；总部全国必须 `30001`；固定小订期不跟顶部日期，车系不影响小订；不新增 RLS、授权集合、缓存或角色推断；`organizationRows` 仅做下钻归因，不过滤摘要分母；未归属目标摘要计入、下钻差额仅内部审计，业务页面不得展示未归属目标提示/警告；内部 audit 保留 `originalCode/canonicalCode/dealerName/target/reason/summaryIncluded/drilldownAttributionStatus/handling` 与 `unassignedTargetTotal/unassignedRows`；发布包仅 q084，staging `test`、shared source `production`；在线登录态浏览器核验未完成 |
| `multi-store-super-app/small-order-model.js` / `small-order-view.js` / `small-order.css` / `small-order-responsive.css` / `small-order-api.js` / `small-order-contract.js` / `validation/small-order-model.test.mjs` / `validation/small-order-api.test.mjs` / `validation/small-order-app-integration.spec.js` / `validation/small-order-a11y-visual.spec.js` / `validation/pc-role-drilldown.spec.js` / 必要时 `index.html` 静态资源版本与构建复制清单 | Phase 3AO MG 07 小订今日新增卡与达成表列 | 已发布 q084，发布后 QA PASS；只新增今日新增展示与表格列。未改变 v2.03 source-row summary、当前范围/RLS、全国 `30001`、未归属目标静默审计、累计小订、目标达成、时间进度、状态、排序、下钻、销售/过程/打铁/导出/单店跳转、发布配置或依赖。AC-390～AC-394 已关闭；最终独立 QA P0/P1/P2=`0/0/0`；Node `225/225`、PC `127/127`、lint `60 files`、build PASS、source-dist `36/36`、v204 cache、`postcss` lock=`8.5.24`、`npm audit --audit-level=high` 为 `0`；发布证据：`operation=update`、`version=0.1.0`、发布日期 `2026-07-29`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`、zip SHA-256=`34c6501b408be9ba67ed0c8a10017d6158e8b6032d78a3662143fe4f90ffd2ca`；线上登录态 UI 待验收；截图证据仅限 `qa-screenshots/mg07-small-order-demo-repair/*fixture*.png` 和 results |
...
- **MG 07 与 MG7 串错：** Phase 3AR 只能把原始值 `MG 07/MG07 EV/MG07 DMH` 映射到销售筛选项 `MG 07`，不得用模糊 like、前缀匹配或去空格归一把 `MG7` 并入；实时试驾不得为了匹配 `MG07 EV` 改用审计字段 `车系名称` 过滤，必须继续以 `闭环车系` 为准。
- **MG 07 小订目标分母串错：** Phase 3AN 后，小订运行时必须复用顶部 `brand / region / district / dealer` 范围，所有层级摘要目标按目标数据集 RLS 可见且匹配顶部范围的目标源行求和。不得新增 RLS、授权集合、授权缓存、角色推断或全国完整性熔断；不得用 valid primary、全量经销商维表、`organizationRows` 或 canonical 可归因集合交集扣分母。总部全国必须为 `30001`，`29714` 必须拦截。v2.03 后未归属目标只做内部审计，业务页面出现未归属提示或警告也视为验收失败。
...
- [x] Phase 3AR 已完成本地修复、两阶段 Code Review 与独立 QA：AC-413～AC-416 已关闭，覆盖邀约历史、邀约实时、试驾历史、试驾实时四源字段，`MG 07/MG07 EV/MG07 DMH` 正向命中、`MG7` 反向排除、current/month/week 三阶段、多选集合、具体车系不 fallback 全部和非目标回归；Object.freeze 三别名，定向 `11/11`、Node `230/230`、lint `62`、build PASS、PC `135/135`、source/dist `36/36`，Code Review Stage1/2 PASS、QA PASS，P0/P1/P2=`0/0/0`；未发布、未 commit/push。
- [ ] Phase 3AK 发布准备中/门禁修复中：MG 07 修复包恢复测试发布仅允许隔离 staging 使用 `environment=test`，共享源码仍为 `production`；第一轮 staging Node `190` 中 `188` 过、`2` 项旧 production 合同冲突失败，尚未调用 `guancli`；全量门禁未过前不得发布。
- [x] Phase 3AN 已发布 q084，发布后 QA PASS：AC-372～AC-389 已实现 MG 07 小订复用顶部 `brand / region / district / dealer` 范围、固定日期独立、车系独立、所有层级 source-row summary、summary 达成分子等于累计小订、总部全国 `30001`、未归属目标摘要计入且下钻差额仅内部审计、业务页面不展示未归属目标提示/警告、内部 audit 保留 `originalCode/canonicalCode/dealerName/target/reason/summaryIncluded/drilldownAttributionStatus/handling` 与 `unassignedTargetTotal/unassignedRows`、未设目标实际计入父级达成分子且门店层“未设目标/--”不判落后、真实阻断错误策略不变、无新增授权请求/缓存、`organizationRows` 不过滤分母、范围不可用同构降级、后台全国 QA 非阻断和 q084-only 发布边界；Review Stage1/2 PASS P0/P1/P2=`0/0/0`，QA PASS P0/P1/P2=`0/0/0`，Node `221/221`，PC `125/125`，lint `60 files`，build PASS，source-dist 关键文件一致/构建复制清单 `36/36`，隐私 `0`，`npm audit` `0`；发布回执 `operation=update`、`fileKey=157e9036-ff39-484f-a1b1-e771d9e2adf7`，zip SHA-256=`0b4041b25118404ed44839cfd29a2a5abaa22204e1df434dbf3e9b6e491591af`；发布后 QA P0/P1/P2=`0/0/1`，P2 仅文档状态已修；在线登录态浏览器核验未完成，未 commit/push。
- [x] Phase 3AO 已发布 q084，发布后 QA PASS：AC-390～AC-394 覆盖今日新增六卡顺序、运行时自然日、固定小订期边界、表格列位置、仅展示不参与达成/排序/下钻，以及顶部日期和车系独立；最终独立 Review Stage 1 P0/P1/P2=`0/0/0`，Stage 2 仅 audit 证据曾初始缺失且后续独立 QA `npm audit --audit-level=high` 为 `0`；最终独立 QA P0/P1/P2=`0/0/0`；Node `225/225`、PC `127/127`、lint `60 files`、build PASS、source-dist `36/36`、v204 cache、`postcss` lock=`8.5.24`。v2.04 可用截图仅限 `qa-screenshots/mg07-small-order-demo-repair/*fixture*.png` 和 results，旧 `qa-screenshots/mg07-small-order-final/` 为历史 v2.03 前截图。发布证据：测试 App `q0844640cf6734877a3193d6` 已于 `2026-07-29` 更新，`operation=update`、`version=0.1.0`、URL=`https://rdata-pv.rauto.com/open-apps/q0844640cf6734877a3193d6/`、zip SHA-256=`34c6501b408be9ba67ed0c8a10017d6158e8b6032d78a3662143fe4f90ffd2ca`；线上登录态 UI 待验收，未 commit/push。
- [x] Phase 3AP 已本地实现、独立 Code Review 与最终 QA 通过，未发布：AC-397～AC-405 已关闭；Code Review Stage 1/2 PASS，QA PASS，P0/P1/P2=`0/0/0`；QA 专项首帧 `8/8`、process `8/8`、iron `12/12`、Node `226/226`、lint `61 files`、PC `135/135`、临时 Vite build PASS、audit critical `0`、diff-check PASS、source/dist `app.js` SHA 一致；1440 浅色销售首屏正常，1280/1440 深色无布局或横向溢出。线上登录态未验收，未发布、未 commit/push；后续技术债为 `data-api` option 直接单测、`app.js/data-api.js` 拆分、线上真实 HAR/登录态性能验收。
- [x] Phase 3AT 已本地修复、Code Review 与 QA 完成，未发布：AC-428～AC-436 已关闭，覆盖投资人空 `dealerCode` / 空组织筛选、5 店目标 `507`、累计/留存/退订 `110/108/2`、一级/叶级映射、字段不一致不伪 `0`、目标不被 `organizationRows` 扣减、固定 MG 07、固定小订期、未配目标实际、scope 内 0 命中/多映射降级、scope 外异常不污染和非目标回归。门禁 Node `235/235`、small-order Playwright `21/21`、lint PASS、build PASS、PC 二次 `137/137`；缓存版本串 P1 已修复并验证，`index.html` 与 `dist/index.html` 三脚本均为 `20260801-mg07-investor-scope-v215`，index-version `3/3`、build PASS；未发布、未 commit/push，线上投资人登录态未验收。
